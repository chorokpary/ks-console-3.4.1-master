...
const GpuVmSelect = (props) => {
  ...
  function toggleVmSelection(vm) {
    ...
    setMessageState({ basic: current.size === 0, unavailable: false, nocapacity: false });
  ...
  function updateRangeSelection(count, startVmId = null) {
    ...
    setNameRange({ start: slice[0]?.name ?? "", end: slice[slice.length - 1]?.name ?? "" });

    if (slice.length < count || availableCount < count) {
      setMessageState({ basic: false, unavailable: hasUnavailable, nocapacity: !hasUnavailable });
    } else {
  ...
  function renderIcon(vmId) {
    ...
    if (status === "unavailable" || status === "error") {
      ...
      return (
        <svg className="w-3 h-3 absolute pointer-events-none" viewBox="0 0 24 24" fill="none">
          <path d="M6 6L18 18" stroke={status === "error" ? "#fff" : "#CCD3DB"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M6 18L18 6" stroke={status === "error" ? "#fff" : "#CCD3DB"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    }
    return null;
          ...
          <div className="vm_grid_left">
              <div className="vm_grid_info_container mt-4">
                <div className="vm_grid_info">
                  {messageState.basic && (
                    <span className="vm_grid_msg_basic text-sm text-gray-700">생성 가능한 가상머신 수를 선택해주세요.</span>
                  )}
                  {messageState.unavailable && (
                    <span className="vm_grid_msg_error_unavailable text-sm text-red-600">이미 사용중인 가상머신이 포함되어 있습니다. 연속된 가상머신 범위를 선택해 주세요.</span>
                  )}
                  {messageState.nocapacity && (
                    <span className="vm_grid_msg_error_nocapacity text-sm text-red-600">입력한 가상머신 수만큼 생성할 수 없습니다. 시작 위치를 확인해 주세요.</span>
                  )}
                  
                  {rangeValue.trim() !== "" &&            
                    <div className="vm_name mt-2">
                      <span className={`vm_start_name ${messageState.unavailable || messageState.nocapacity ? "text-red-600" : ""}`}>{props.vmPrefix}{nameRange.start}</span>
                      <span className={`vm_dash mx-2`}>{nameRange.end !== "" ? "-" : ""}</span>
                      <span className={`vm_end_name ${messageState.unavailable || messageState.nocapacity ? "text-red-600" : ""}`}>{props.vmPrefix}{nameRange.end}</span>
                    </div>
                  }   
                </div>
              </div>
          </div>
          <div className="vm_grid_right">
              <Button
