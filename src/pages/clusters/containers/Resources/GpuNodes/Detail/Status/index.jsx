...
export default class Status extends React.Component {
  ...
  fnGetData = async () => {
    ...
    const project = vmListData.filter(
      item => item.node === this.store.detail.name
    )[0]?.project
    const vmJoinData =
      vmListData
        .filter(item => item.node === this.store.detail.name)
        .map(item => item.name)
        .join('|') || ''
  ...
  renderDeployments() {
    ...
    const container_toolkit = {
      name: 'container_toolkit',
      flag: deploy.container_toolkit,
    }
    ...
    const operator_validator = {
      name: 'operator_validator',
      flag: deploy.operator_validator,
    }
    const mig_manager = { name: 'mig_manager', flag: deploy.mig_manager }
    const sandbox_device_plugin = {
      name: 'sandbox_device_plugin',
      flag: deploy.sandbox_device_plugin,
    }
    const cc_manager = { name: 'cc_manager', flag: deploy.cc_manager }
    const vfio_manager = { name: 'vfio_manager', flag: deploy.vfio_manager }
    const sandbox_validator = {
      name: 'sandbox_validator',
      flag: deploy.sandbox_validator,
    }
    const vgpu_manager = { name: 'vgpu_manager', flag: deploy.vgpu_manager }
    const vgpu_device_manager = {
      name: 'vgpu_device_manager',
      flag: deploy.vgpu_device_manager,
    }
      ...
      <>
        <Panel title={t('RESOURCES_GPU_CONTAINER_DEPLOYMENT_STATUS')}>
          <div className={styles.deployments}>
            ...
            <DeploymentCard
              key="operator_validator"
              data={operator_validator}
            />
        ...
        <Panel title={t('RESOURCES_GPU_VM_PASSTHROUGH_DEPLOYMENT_STATUS')}>
          <div className={styles.deployments}>
            ...
            <DeploymentCard
              key="sandbox_device_plugin"
              data={sandbox_device_plugin}
            />
        ...
        <Panel title={t('RESOURCES_GPU_VM_VGPU_DEPLOYMENT_STATUS')}>
          <div className={styles.deployments}>
            ...
            <DeploymentCard
              key="vgpu_device_manager"
              data={vgpu_device_manager}
            />
            <DeploymentCard
              key="sandbox_device_plugin"
              data={sandbox_device_plugin}
