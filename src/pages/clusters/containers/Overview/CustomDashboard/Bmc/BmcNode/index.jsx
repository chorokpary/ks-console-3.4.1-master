...
const BmcNode = ({ x, y, w, h, nodeData, cluster }) => {
  ...
  const [metricStateObj, setMetricStateObj] = useState({
    total: 0,
    on: 0,
    off: 0,
    unknown: 0,
  ...
  const getMetricValue = (metricData, data, type) => {
    const instance = toJS(
      data.system_type == 'C' ? data.name : data.nodeExporter.ip
    )
    const target = data.openBMC?.address

    const metrics =
      type == 'redfish'
        ? metricData.find(item => get(item, 'metric.target') === target)
        : data.system_type == 'C'
  ...
  const getType = data => {
    ...
    const type_data = metricType.find(
      item => get(item, 'metric.instance').split(':')[0] === instance
    )
  ...
  const getMemory = data => {
    ...
    return (
      <>
        <p>{memory_percent}%</p>
        <span>
          {memory_used}Gi/{memory_total}Gi
        </span>
      </>
    )
  }
  ...
  const getDisk = data => {
    ...
    const disk_percent = isNaN(((disk_used / disk_total) * 100).toFixed(0))
      ? 0
  ...
  const handleList = nodeType => {
    ...
    if (nodeType == '') {
      ...
      nodeData.map(obj => {
        const instance = toJS(
          obj.system_type == 'C' ? obj.name : obj.nodeExporter.ip
        )
        const type_data = metricType.find(
          item => get(item, 'metric.instance').split(':')[0] === instance
        )
      ...
      <div className="grid_item">
        <div className="grid_title" style={{ cursor: 'default' }}>
          ...
          <div className="view-result">
            {t('RESOURCES_TOTAL')} {nodeList.length}
            {t('RESOURCES_COUNT_GUN')}
          </div>
                ...
                <div className="box type_status">
                  <div className="cont_group">
                    <div className="cont1">
                      <div className="number_wrap">
                        <p>
                          <span className="em">{metricStateObj.on}</span> /{' '}
                          {metricStateObj.total}
                        </p>
                      </div>
                    </div>
                    <div className="cont2">
                  ...
                  <div className="hexagon_wrap">
                    {nodeList.map((obj, idx) => (
                      <div
                        key={idx}
                        className={`hexagon ${getState(metricState, obj)}`}
                      >
                        <span>
                          {getType(obj).toUpperCase() === 'CLUSTERNODE'
                            ? 'etc'
                            : getType(obj).toUpperCase()}
                        </span>
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="box-radius none-shadow">
                      <table className="tbl_list">
                        ...
                        <thead>
                          <tr>
                            <th>
                              <strong>{t('RESOURCES_BAREMETAL_NODE')}</strong>
                            </th>
                            <th>
                              <strong>CPU</strong>
                            </th>
                            <th>
                              <strong>{t('RESOURCES_MEMORY')}</strong>
                            </th>
                            <th>
                              <strong>{t('RESOURCES_DISK')}</strong>
                            </th>
                            <th>
                              <strong>{t('RESOURCES_POWER')}</strong>
                            </th>
                            <th>
                              <strong>{t('RESOURCES_TEMPERRATURE')}</strong>
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            nodeList.map((obj, idx) => (
                              <tr key={idx}>
                                <td className="tbl_tit">
                                  <i
                                    className={`ico-type24-${getType(
                                      obj
                                    )} ${getState(metricState, obj)}`}
                                  ></i>
                                  <p>{obj.name}</p>
                                </td>
                                <td>
                                  <p>
                                    {Math.round(
                                      getMetricValue(metricCpu, obj)
                                    ).toFixed(1)}
                                    %
                                  </p>
                                  <span>
                                    {getMetricValue(metricCore, obj)}core
                                  </span>
                                </td>
                                <td>{getMemory(obj)}</td>
                                <td>{getDisk(obj)}</td>
