...
const ResourcesUsage = ({ widgetKey, monitorStore, ...props }) => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      ...
      // kaas list
      const kaasVmList = await resourceStore.fetchMachinesAll({ limit: 1000, ...props })
      let promsql_pod_kaas_vm_list = ''
      let kaas_vm_list_length = 0
      kaasVmList.map(obj => {
        const kaasVmName = get(obj, 'name')
        promsql_pod_kaas_vm_list +=
          promsql_pod_kaas_vm_list != '' ? '|' + kaasVmName + '.*' : kaasVmName + '.*'
        kaas_vm_list_length++
      })
      ...
      const vmCpuData = await customStore.fetchMetric({
        expr: `sum(100 - (avg by (pod) (irate(node_cpu_seconds_total{service="launcher-node-exporter",mode="idle",pod=~"${promsql_pod_vm_list}"}[${step}])) * ${times})) / 100 / ${vm_list_length}`,
      ...
      const kaasCpuData = await customStore.fetchMetric({
        expr: `sum(100 - (avg by (pod) (irate(node_cpu_seconds_total{service="launcher-node-exporter",mode="idle",pod!~"${promsql_pod_kaas_vm_list}"}[${step}])) * ${times})) / 100 / ${kaas_vm_list_length}`,
        start: currentTime - 30000,
        end: currentTime,
        cluster: props.cluster,
      })
