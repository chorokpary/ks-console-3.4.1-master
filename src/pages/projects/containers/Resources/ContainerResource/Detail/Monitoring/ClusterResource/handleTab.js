...
function getVmData(data) {
  let cpuCnt = 0
  data.cpuData.forEach(obj => {
    cpuCnt += Number(last(obj.values)[1])
  })
  let memoryCnt = 0
  data.memoryData.forEach(obj => {
  ...
  return [
    {
      ...
      title: 'CPU_USAGE',
      unit: '%',
      legend: data.cpuData.map(item => item.metric.pod),
      data: data.cpuData.map(item => item),
    },
    {
      activeTab: 'memory',
      type: 'utilisation',
      ...
      unit: '%',
      unitType: 'memory',
      legend: data.memoryData.map(item => item.metric.pod),
      data: data.memoryData.map(item => item),
    },
  ]
}
