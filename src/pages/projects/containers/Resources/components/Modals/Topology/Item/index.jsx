...
const TopologyItem = props => {
  ...
  useEffect(() => {
    var wrapHeight = document.querySelector('.network_element_wrap')
      .offsetHeight
    wrapHeight += 100
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setNetworkList(
        store.networkList.filter(item => item.project == namespace)
      )
      setSriovList(store.sriovList)
      setRouterList(store.routerList.filter(item => item.project == namespace))
      setFloatingList(
        store.floatingList.filter(item => item.project == namespace)
      )
      setLoadBalancerList(
        store.loadbalancerList.filter(item => item.project == namespace)
      )

      const internalNetworkList = store.networkList?.filter(
        row => row.external == false && row.project == namespace
      )
      const externalNetworkList = store.networkList?.filter(
        row => row.external == true && row.project == namespace
      )
  ...
  const renderLeftMenu = () => {
    return (
      <ul className="element_overview">
        <li>
          <i className="ico-type24-network">
            <span className="popover above">네트워크</span>
          </i>
          <div className="text-wrapper">{internalList.length}</div>
        </li>
        <li>
          <i className="ico-type24-externalnetwork">
            <span className="popover above">외부네트워크</span>
          </i>
          <div className="text-wrapper">{externalList.length}</div>
        </li>
        <li>
          <i className="ico-type24-sriov">
            <span className="popover above">SR-IOV</span>
          </i>
          <div className="text-wrapper">{sriovList.length}</div>
        </li>
        <li>
          <i className="ico-type24-vm">
            <span className="popover above">가상머신</span>
          </i>
          <div className="text-wrapper">{vmList.length}</div>
        </li>
        <li>
          <i className="ico-type24-router">
            <span className="popover above">라우터</span>
          </i>
          <div className="text-wrapper">{routerList.length}</div>
        </li>
        <li>
          <i className="ico-type24-floatingip">
            <span className="popover above">플로팅 IP</span>
          </i>
          <div className="text-wrapper">{floatingList.length}</div>
        </li>
        <li>
          <i className="ico-type24-loadbalancer">
            <span className="popover above">로드밸런서</span>
          </i>
      ...
      return (
        <li className="network_bar">
          <div className="network_name">
            <div className={`hexagon_circle color_${colorNum}`}></div>
            <i className={`ico-type-${networkIcon}-wh`}></i>
            <div className="name">
              <em>{ternalType}</em>
              <span>{cidr}</span>
            </div>
          </div>
          <div className={`network_line color_${colorNum}`}>
            <span>{obj.name}</span>
          </div>
        </li>
      )
          ...
          if (!vmArray.includes(vm.name)) {
            ...
            return (
              <div className={`network_element leftBar`}>
                <div
                  className="element left leftMargin"
                  style={{
                    '--marginleft': (num == 1 ? 0 : 370 * (num - 1)) + 'px',
                  }}
                >
                  <div className={`line ${bondingLeft} color_${colorNum}`}>
                    <span>{vmNetworkIp}</span>
                  </div>
                </div>
                <div className="element info">
                  ...
                  <p>
                    ...
                    <span>
                      {vm.name}
                      <a
                        href={`/${workspace}/clusters/${cluster}/projects/${namespace}/vms/${vm.name}`}
                        className="btn_go"
                        onClick={() => closeModal()}
                      ></a>
                    </span>
          ...
          if (!roterArray.includes(router.name)) {
            ...
            return (
              <div className={`network_element leftBar`}>
                ...
                >
                  <div className={`line color_${colorNum}`}>
                    <span>{vrouterIp}</span>
                  </div>
                </div>
                <div className="element info">
                  ...
                  <p>
                    ...
                    <span>
                      {router.name}
                      <a
                        href={`/${workspace}/clusters/${cluster}/projects/${namespace}/routers/${router.name}`}
                        className="btn_go"
                        onClick={() => closeModal()}
                      ></a>
                    </span>
                ...
                <div className="element info">
                  ...
                  <p>
                    ...
                    <span>
                      {load.name}
                      <a
                        href={`/${workspace}/clusters/${cluster}/projects/${namespace}/loadBalancers/${load.name}`}
                        className="btn_go"
                        onClick={() => closeModal()}
                      ></a>
                    </span>
