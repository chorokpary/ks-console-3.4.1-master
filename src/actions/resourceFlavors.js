...
import RegistModal from 'pages/clusters/containers/Resources/components/Modals/Flavors/Regist'
import ModifyModal from 'pages/clusters/containers/Resources/components/Modals/Flavors/Modify'
  ...
  'flavor.regist': {
    on({ store, cluster, workspace, namespace, success, devops, ...props }) {
      const modal = Modal.open({
        ...
        title: t('RESOURCES_CREATE_FLAVOR'),
        modal: RegistModal,
        store,
        cluster,
        workspace,
  ...
  'flavor.remove': {
    on({
      ...
      const modal = Modal.open({
        ...
        desc: t.html('RESOURCES_DELETE_FLAVOR_TIP', {
  ...
  'flavor.remove.batch': {
    on({ store, cluster, workspace, namespace, success, devops, ...props }) {
      ...
      const modal = Modal.open({
        ...
        desc:
          usernames.split(', ').length === 1
            ? t.html('RESOURCES_DELETE_FLAVOR_TIP', { resource: usernames })
            : t.html('RESOURCES_DELETE_FLAVOR_TIP', { resource: usernames }),
        resource: usernames,
        store,
        ...props,
