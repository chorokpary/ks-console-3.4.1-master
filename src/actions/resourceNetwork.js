  ...
  'networks.remove': {
    on({
      ...
      const modal = Modal.open({
        ...
        modal: DeleteModal,
        title: t('삭제'),
        desc: t.html('네트워크의 이름을 입력하여 이 작업의 위험을 이해하고 있는지 확인합니다.', {
  ...
  'networks.remove.batch': {
    on({ store, cluster, workspace, namespace, success, devops, ...props }) {
      ...
      const modal = Modal.open({
        ...
        desc:
          usernames.split(', ').length === 1
            ? t.html('네트워크의 이름을 입력하여 이 작업의 위험을 이해하고 있는지 확인합니다.', { resource: usernames })
            : t.html('네트워크들의 이름을 입력하여 이 작업의 위험을 이해하고 있는지 확인합니다.', { resource: usernames }),
        resource: usernames,
        store,
        ...props,
      })
