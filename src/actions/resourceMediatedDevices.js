...
import RegistModal from 'pages/clusters/containers/Resources/components/Modals/MediatedDevices/Regist'
import ModifyModal from 'pages/clusters/containers/Resources/components/Modals/MediatedDevices/Modify'

import EditYamlModal from 'components/Modals/EditYaml'
import DeleteModal from 'components/Modals/Delete'

export default {
  'mediatedDevice.regist': {
  ...
  'mediatedDevice.remove': {
    on({
      ...
      const modal = Modal.open({
        ...
        modal: DeleteModal,
        title: t('삭제'),
        desc: t.html('Mediated 디바이스 이름 입력하여 이 작업의 위험을 이해하고 있는지 확인합니다.', {
  ...
  'mediatedDevice.remove.batch': {
    on({ store, cluster, workspace, namespace, success, devops, ...props }) {
      ...
      const modal = Modal.open({
        ...
        desc:
          usernames.split(', ').length === 1
            ? t.html('Mediated 디바이스 이름 입력하여 이 작업의 위험을 이해하고 있는지 확인합니다.', { resource: usernames })
            : t.html('Mediated 디바이스 이름 입력하여 이 작업의 위험을 이해하고 있는지 확인합니다.', { resource: usernames }),
        resource: usernames,
        store,
        ...props,
