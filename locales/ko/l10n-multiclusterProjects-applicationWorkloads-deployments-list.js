...
module.exports = {
  // Banner
  WORKLOAD_DESC:
    '워크로드는 서비스 요청을 처리하는 데 사용되며 하나 이상의 파드를 포함할 수 있습니다. 로깅 및 모니터링과 같은 시스템 기능도 워크로드에 의해 구현됩니다.',
  // List
  ...
  SPECIFY_WEIGHTS_DESC:
    '각 클러스터의 총 파드 복제본 수와 가중치를 설정합니다. 파드 복제본은 가중치에 따라 클러스터에 스케줄링 됩니다.',
  ...
  IMAGE_DESC:
    '개인 이미지 레지스트리를 사용하려면 먼저 이미지 레지스트리 시크릿을 만들어야 합니다. <a href={link} target="_blank">자세히 알아보기 </a>',
  ...
  IMAGE_PULL_POLICY_ALWAYS_DESC:
    '파드가 생성되거나 업데이트될 때 항상 이미지를 Pulling합니다.',
  IMAGE_PULL_POLICY_IFNOTPRESENT_DESC:
    '필요한 이미지가 로컬에 없는 경우에만 이미지를 Pull합니다.',
  IMAGE_PULL_POLICY_NEVER_DESC:
    '로컬 이미지만 사용합니다. 필요한 이미지가 로컬로 존재하지 않으면 컨테이너가 비정상적으로 \b동작할 수 있습니다.',
  ...
  LIFECYCLE_MANAGEMENT_DESC:
    '환경 준비 또는 정상 종료를 위해 컨테이너가 시작된 후 또는 중지되기 전까지 수행할 작업을 추가합니다.',
  ...
  CREATE_CONFIGMAP_SECRET_DESC:
    '요구 사항을 충족하는 컨피그맵 또는 시크릿이 없는 경우',
  CREATE_CONFIG: '컨피그맵 생성',
  OR: '또는',
  CREATE_SECRET: '시크릿을 생성합니다.',
  DEFAULT_REPOSITORY: '기본 레지스트리',
  SET_DEFAULT_REPOSITORY: '기본 레지스트리로 설정',
  SET_AS_DEFAULT_REPOSITORY_DESC:
    '이미지 레지스트리를 기본 이미지 레지스트리로 설정합니다. 달리 지정하지 않은 경우 시스템은 기본 이미지 레지스트리의 이미지를 사용하여 응용 프로그램 워크로드를 만듭니다. 각 프로젝트에는 기본 이미지 레지스트리가 하나만 허용됩니다.',
  SET_AS_DEFAULT_REPOSITORY: '기본값으로 설정',
  SET_DEFAULT_REPO_SUCCESSFUL: '기본 레지스트리로 설정 성공',
  ...
  PRIVILEGED_MODE_DESC:
    '호스트의 루트 사용자로 컨테이너 프로세스를 실행합니다.',
  ALLOW_PRIVILEGE_ESCALATION: '권한 상승 허용',
  ALLOW_PRIVILEGE_ESCALATION_DESC:
    '컨테이너 프로세스가 상위 프로세스보다 더 많은 권한을 획득할 수 있습니다. 이 옵션은 특권 모드가 활성화된 경우 기본적으로 활성화됩니다.',
  ROOT_DIRECTORY_READONLY: '루트 디렉터리 읽기 전용',
  ROOT_DIRECTORY_READONLY_DESC:
    '컨테이너 파일 시스템의 루트 디렉터리를 읽기 전용으로 설정합니다.',
  USER_AND_USER_GROUP: '사용자 및 사용자 그룹',
  USER_GROUP: '사용자 그룹',
  RUN_AS_NON_ROOT: '루트가 아닌 사용자 권한으로 실행',
  RUN_AS_NON_ROOT_DESC:
    '컨테이너를 시작하기 전에 루트 사용자 권한으로 컨테이너를 실행할지 여부를 확인합니다. 예를 선택하면 컨테이너가 시작되지 않습니다.',
  RUN_AS_USER_DESC:
    '컨테이너 프로세스의 진입점을 실행하는 UID입니다. 기본값은 이미지 메타데이터에 지정된 UID입니다.',
  RUN_AS_USER_GROUP_DESC:
    '컨테이너 프로세스의 진입점을 실행하는 GID입니다. 기본값은 컨테이너 런타임에 지정된 GID입니다.',
  ...
  SYNC_HOST_TIMEZONE_DESC:
    '컨테이너의 표준 시간대를 호스트의 표준 시간대와 동기화합니다.',
  ...
  ROLLINGUPDATE_DESC:
    '오래 된 파드 복제본을 점차 새로운 드 복제본으로 교체합니다. 업데이트 프로세스 중에는 서비스가 중단되지 않습니다.',
  SIMULTANEOUS_UPDATE_DESC:
    '새로운 파드 복제본을 만들기 전에 기존 파드 복제본을 모두 삭제합니다. 업데이트 프로세스 중에 서비스가 중단됩니다.',
  ENTER_INTEGER_OR_PERCENTAGE: '정수 또는 백분율을 입력하십시오.',
  MAX_EXTRA_EMPTY:
    '업데이트 프로세스 중 허용되는 여분의 파드 복제본의 최대 수 또는 백분율을 설정하십시오.',
  ...
  POD_SECURITY_CONTEXT_TIP:
    '사용자, 사용자 그룹 및 SELinux 컨텍스트 설정이 파드 보안 컨텍스트 및 컨테이너 보안 컨텍스트에 모두 정의된 경우 컨테이너 보안 컨텍스트 설정이 파드 보안 컨텍스트 설정을 재정의합니다.',
  // List > Create > Pod Settings > Pod Scheduling Rules
  POD_SCHEDULING_RULES: '파드 스케줄링 규칙',
  POD_SCHEDULING_RULES_DESC:
    '노드에 파드 복제본을 스케줄링 위한 규칙을 지정합니다.',
  DEFAULT_RULES: '기본 규칙',
  ...
  DECENTRALIZED_SCHEDULING_DESC:
    '가능한 경우 파드 복제본을 다른 노드로 스케줄링합니다.',
  CENTRALIZED_SCHEDULING_DESC:
    '가능한 경우 파드 복제본을 동일한 노드로 스케줄링합니다.',
  ...
  POD_GRACE_PERIOD: '파드 유예 기간',
  POD_GRACE_PERIOD_DESC:
    '파드가 종료될 때까지 유예 기간을 설정하여 유예 기간 이후 파드가 강제 종료 되도록 합니다.',
  TERMINATION_GRACEPERIOD_SECONDS: '종료 유예 기간(초)',
  ...
  WORKLOAD_MOUNT_VOLUME_DESC:
    '퍼시스턴트 볼륨, 임시 볼륨 또는 HostPath 볼륨을 컨테이너에 마운트합니다.',
  SELECT_PERSISITENT_VOLUME_CLAIM: '퍼시스턴트 볼륨 클레임 선택',
  SELECT_PERSISITENT_VOLUME_CLAIM_DESC:
    '퍼시스턴트 볼륨 클레임에 따라 생성된 퍼시스턴트 볼륨을 컨테이너에 마운트합니다.',
  CAPACITY: '용량',
  ...
  HOSTPATH_TIP:
    'HostPath 볼륨을 사용하여 호스트 파일 시스템의 파일 또는 디렉토리를 컨테이너에 마운트합니다.',
  HOST_PATH: 'Host Path',
  ...
  SELECT_NODES: '노드 선택',
  SELECT_NODES_DESC:
    '파드 복제본을 특정 노드에 할당합니다. 레이블을 사용하여 노드를 선택하거나 수동으로 노드를 지정할 수 있습니다.',
  ADD_NODE_SELECTOR: '노드 셀렉터 추가',
  ADD_NODE_SELECTOR_TIP: '올바른 키와 값을 입력하세요.',
  ...
  CLUSTER_DIFF_CONTAINER_SETTINGS_DESC:
    '서로 다른 클러스터에서 서로 다른 컨테이너 설정을 적용합니다.',
  CLUSTER_DIFF_PORT_SETTINGS_DESC:
    '서로 다른 클러스터에 있는 컨테이너에 대해 서로 다른 포트를 설정합니다.',
  CLUSTER_DIFF_ENVIRONMENT_VARIABLES_DESC:
    '서로 다른 클러스터에 있는 컨테이너에 대해 서로 다른 환경 변수를 설정합니다.',
